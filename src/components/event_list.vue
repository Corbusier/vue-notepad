<template>
	<div class="event-content">
		...
		<div class="event-box">
			<ul>
				<li class="event-list" v-for="value of getToDo">
		...
		<div class="event-box">
			<ul>
				<li class="event-list" v-for="value of getDone">
					<input checked type="checkbox" @click="moveTodo(value.id)">
					<div>{{ value.content }}</div>
		...
		<div class="event-box">
			<ul>
				<li class="event-list" v-for="value of getCancel">
	...
	export default{
		...
		methods: {
			changeCollapse(num,event){
				...
				show == true ? this.closeCollapase(event):
				this.openCollapse(event)

				this.collapse[num].show = !show 
			},
			closeCollapase(event) {//关闭折叠面板
