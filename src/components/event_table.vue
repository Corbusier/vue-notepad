	...
	<section class="event-all" :class="{'event-all-show': isShow}">
		<div class="table-box">
			...
			<div class="screen-box">
				<div class="div-select" :class="{'active': active}">
					...
					<div class="div-select-body">
						<div class="div-option" @click="doSelect(0)"> 
							筛选类型
						</div>
						<div class="div-option" @click="doSelect(1)"> 
							未完成
						</div>
						<div class="div-option" @click="doSelect(2)"> 
							已完成
						</div>
						<div class="div-option" @click="doSelect(3)"> 
							已取消
						</div>
					</div>
				</div>
				<input class="div-search" v-model="screen_title"  placeholder="筛选关键词">
			</div>
			<table class="event-table">
				...
				<tbody>
					<!-- 列表过滤 -->
                    <tr v-for="(item, index) of notepad">
...
<script>
	export default {
        ...
        computed: {
            notepad(){
                let self = this;
                return self.$store.getters.getEventList.filter(        (item)  => {
